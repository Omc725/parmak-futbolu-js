...
import { AI_SPEED_MULTIPLIERS } from '../constants.js';
...
const GameCanvas = ({
  ...
  const gameState = useRef({
    ...
    ballTrail: [],
    confettiParticles: [],
    goalShake: { side: null, progress: 0 },
  ...
  const resetPositions = useCallback((goalScorer) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const gs = gameState.current;
  ...
  const draw = useCallback(() => {
    ...
    if (player1 && player2 && ball) {
      ...
      const drawSoccerBall = (x, y, r, rotation) => {
  ...
  const update = useCallback(() => {
    ...
    const handlePaddleCollision = (player, isTopPlayer) => {
